...
def ball_pivoting_mesh_reconstruct(pcd, radii = [0.001, 0.002, 0.004, 0.008], visualize=False):
    ...
    if visualize:
        ...
        plt.add_mesh(point_cloud, color='k', point_size=10)
        plt.add_mesh(pymesh)
        plt.add_title('ball_pivoting_reconstruction')
        plt.show()
        # visualize_o3d([pcd, mesh], title='alpha_shape_reconstruction')
...
# @profile
def poisson_mesh_reconstruct(cube, rest=None, depth=2, mesh_fix=False, visualize=False):
...
def point_cloud_to_image(pcds, width=800, height=600, fov_step=90, distance=1.0):
    vis = o3d.visualization.Visualizer()
    vis.create_window(width=width, height=height)
    
    # add the list of point clouds to the visualizer
    map(lambda pcd: vis.add_geometry(pcd), pcds)
