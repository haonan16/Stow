...
if __name__ == "__main__":
    ...
    parser.add_argument("--environment", type=str, default="StowInsert", help="Task to perform, e.g.: 'Lift', 'TwoArmPegInHole', 'NutAssembly', etc.")
    ...
    parser.add_argument("--device", type=str, default="spacemouse", help="Choice of input device. Can be 'spacemouse' or 'keyboard'")
    parser.add_argument("--pos-sensitivity", type=float, default=5.0, help="How much to scale position user inputs")
    ...
    if 'Stow' in args.environment:
        from config.config import gen_args
        kwargs={'args':gen_args()}
        # Create environment
        env = suite.make(
            **config,
            has_renderer=True,
    ...
    while True:
        ...
        while True:
            ...
            i=i+1
            if i%20==0:
                print(f"end effector position:{obs['robot0_eef_pos']}, quat:{obs['robot0_eef_quat']},\n",
                      f"euler(rxyz):{trans.mat2euler(trans.quat2mat(obs['robot0_eef_quat']), 'rxyz')}, euler(sxyz):{trans.mat2euler(trans.quat2mat(obs['robot0_eef_quat']), 'sxyz')}\n",
                      f"axisangle: {trans.quat2axisangle(obs['robot0_eef_quat'])}\n")
            env.render()
