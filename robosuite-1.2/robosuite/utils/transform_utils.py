...
def euler2mat(euler, use_rad=True):
    """
    ...
    if use_rad:
        ...
        ai, aj, ak = -np.deg2rad(euler[..., 2]), -np.deg2rad(euler[..., 1]), -np.deg2rad(euler[..., 0])
...
def quat2mat(quaternion):
    ...
    Args:
        quaternion (np.array or torch.Tensor): (x,y,z,w) vec4 float angles

    Returns:
        np.array or torch.Tensor: 3x3 rotation matrix
    """
    if isinstance(quaternion, torch.Tensor):
        device = quaternion.device
        inds = torch.tensor([3, 0, 1, 2], device=device)
        q = quaternion.index_select(0, inds).clone()
        ...
        return torch.tensor(
            ...
            dtype=quaternion.dtype, device=device
        )
    else:
    ...
    # else:
    #     raise ValueError("quaternion must be either torch.Tensor or numpy.ndarray")
...
def quat2axisangle(quat):
    ...
    if isinstance(quat, torch.Tensor):
        # Clip quaternion
        quat_clamped = torch.cat((quat[..., :3], torch.clamp(quat[..., 3:4], -1, 1)), dim=-1)

        # Compute the denominator
        den = torch.sqrt(1 - quat_clamped[..., 3] * quat_clamped[..., 3])

        # Handle zero degree rotation
        zero_rotation_mask = torch.isclose(den, torch.tensor(0.)).unsqueeze(-1).expand_as(quat_clamped[..., :3])

        # Compute axis-angle representation
        axis_angle = (quat_clamped[..., :3] * 2 * torch.acos(quat_clamped[..., 3]).unsqueeze(-1)) / den.unsqueeze(-1)
        axis_angle[zero_rotation_mask] = 0

        return axis_angle
    elif isinstance(quat, np.ndarray):
        # Clip quaternion
        quat_clamped = np.concatenate((quat[..., :3], np.clip(quat[..., 3:4], -1, 1)), axis=-1)

        # Compute the denominator
        den = np.sqrt(1 - quat_clamped[..., 3] * quat_clamped[..., 3])

        # Handle zero degree rotation
        zero_rotation_mask = np.isclose(den, 0.).reshape(-1, *([1] * (len(quat.shape) - 2)), 1)

        # Compute axis-angle representation
        axis_angle = (quat_clamped[..., :3] * 2 * np.arccos(quat_clamped[..., 3]).reshape(-1, *([1] * (len(quat.shape) - 2)), 1)) / den.reshape(-1, *([1] * (len(quat.shape) - 2)), 1)
        axis_angle = np.where(zero_rotation_mask, 0, axis_angle)

        return axis_angle
    else:
        raise ValueError("quat must be either torch.Tensor or numpy.ndarray")
...
def get_quat_error(target_quat, cur_quat):
    '''calculates the angle error between two quaternions
    reference: http://www.boris-belousov.net/2016/12/01/quat-dist/
    Args:
        target_quat (np.array): (x, y, z, w) desired quaternion orientation
        cur_quat (np.array): (x, y, z, w) current quaternion orientation
    '''
    if len(target_quat.shape)>1:
        target_quat = target_quat.squeeze()
    # get the quaternion in (w, x, y, z) format
    ...
    # calculate the dot product and use the arcosine to get the angle in radians
    angle_diff = 2 * np.arccos(np.abs(np.dot(target_quat, cur_quat)))
    return angle_diff


# def quaternion_slerp(quat1, quat2, percentage):
#     """
#     Spherical linear interpolation between two quaternions.
    
#     Parameters:
#         q1 (numpy.ndarray): First quaternion (shape: 4).
#         q2 (numpy.ndarray): Second quaternion (shape: 4).
#         t (float): Interpolation factor between 0 and 1.
    
#     Returns:
#         interpolated_q (numpy.ndarray): Interpolated quaternion (shape: 4).
#     """
#     # Create a Rotation object from the input quaternions
#     rotation1 = R.from_quat(quat1)
#     rotation2 = R.from_quat(quat2)

#     # Perform Slerp interpolation
#     slerp_rotation = rotation1.slerp([percentage], rotation2)
    
#     # Convert the result back to a quaternion
#     interpolated_quat = slerp_rotation.as_quat()[0]
    
#     return interpolated_quat

def quaternion_lerp(quat1, quat2, percentage):
    # Perform the linear interpolation
    interpolated_quat = quat1 * (1 - percentage) + quat2 * percentage
    if isinstance(quat1, torch.Tensor):
        interpolated_quat /= torch.norm(interpolated_quat)
    else:
